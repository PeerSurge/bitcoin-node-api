...
module.exports = function(){
  ...
  function express_app(){
    ...
    app.get('*', hasAccess, function(req, res){
      ...
      var pathParams = pathParts.slice(1).filter(function(s){ return s.length > 0; });
    ...
    function hasAccess(req, res, next){
      ...
      var method = req.path.substring(1).split('/')[0];
      if('undefined' == typeof accesslist[method]){
        if(accesslist.type == 'only') res.status(403).end('This method is restricted.');
        else return next();
      }
      else{
        ...
        else res.status(403).end('This method is restricted.');
      } 
    }
  ...
  function setWalletDetails(details){
    // Allow direct assignment of a mock client for testing
    if (typeof details.command === 'function') {
      client = details;
    } else if('undefined' == typeof details.rpc){
